"""Даша Database — SQLite async (aiosqlite), WAL mode."""
import logging, time
from typing import List, Optional
import aiosqlite
from bot.config import config

logger = logging.getLogger("dasha.db")
...
async def load_posted_news_from_file():
    ...
    try:
        ...
        if os.path.exists(filepath):
            ...
            if count > 0:
                import logging
                logging.getLogger("dasha.db").info(f"Loaded {count} posted_news from file backup")
    except Exception as e:
        pass
