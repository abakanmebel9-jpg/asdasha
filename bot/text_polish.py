...
# Раунд 13: буллет-лейбл «• Слово:» в начале строки — жирным маркер с точкой.
# Покрывает чек-листы и сравнения из статик-банка и AI («• Фасады: зазоры ровные»)
_BULLET_LABEL_PAT = re.compile(r"(^•\s*)([^•\n:]{1,32}?)(:)(?=\s)")
...
def stylize_post_html(text: str) -> str:
    ...
    for line in lines:
        ...
        # Раунд 13: буллет-лейбл «• Слово:» — жирным (вместе с маркером)
        styled = _BULLET_LABEL_PAT.sub(r"<b>\1\2\3</b>", styled)
        # «Шаг 1» жирным
