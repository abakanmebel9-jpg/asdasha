"""Даша Proactive — topic starter + conversation summaries."""
...
logger = logging.getLogger("dasha.proactive")
...
TOPIC_STARTERS = ["народ, что думаете про", "кто-нибудь следил за", "а вы как считаете насчёт", "кстати, слышали что", "интересно ваше мнение —", "недавно думала про", "народ, а кто-нибудь", "блин, только что вспомнила —", "кстати о чём говорили,", "а что нового у всех? давно не общались"]
GENERAL_TOPICS = ["тренды в мебели и интерьере 2026", "какие фасады практичнее — матовые или глянцевые", "кухня: керамогранит против плитки на фартуке", "шкаф-купе против распашного: плюсы и минусы", "где заказать мебель и как не нарваться на халтуру", "ошибки планировки кухни, которые видны сразу", "массив дерева против МДФ: что выбрать на фасады", "как организовать гардеробную: системы хранения", "цвет кухни: смелые решения против классики", "уход за мебелью: лайфхаки от дизайнера", "маленькая прихожая: как уместить всё нужное", "подсветка на кухне: must have или лишнее", "столешница: камень, HPL или массив", "ремонт и мебель: что сначала — стены или заказ"]
...
async def _check_and_start_topic(chat_id):
    ...
    try:
        ...
        if sent:
            await db.add_group_message(chat_id=chat_id, user_id=config.BOT_ID, username=config.BOT_USERNAME.lstrip("@"), first_name="Даша", content=text, is_media=False, is_bot=True)
