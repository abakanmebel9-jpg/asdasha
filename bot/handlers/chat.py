"""Даша Private chat handler — normal AI conversation with memory."""
...
async def cmd_start(message):
    ...
    await message.reply("Привет! Я Даша 😊 Можно просто на «ты». Расскажи что-нибудь — поболтаем? ☕")
...
async def handle_private_photo(message):
    ...
    try:
        ...
        if data_uri:
            from bot.persona import PERSONA_PROMPT
            vision_prompt = f"Пользователь прислал фото. Опиши что видишь (1-2 предложения), потом живо отреагируй как Даша. {'Подпись: ' + caption if caption else ''}"
            mood = await current_mood_descriptor()
            system = PERSONA_PROMPT + f"\n\nТвоё текущее настроение: {mood}."
...
async def _run_consultation(message: Message, consult_type: str):
    ...
    try:
        ...
        if consult_type == "matrix":
            ...
            system = PERSONA_PROMPT + f"\n\nТы Даша. Настроение: {mood}. Делаешь профессиональный разбор Матрицы Судьбы. Женский род."
            prompt = f"Сделай разбор Матрицы Судьбы для даты {day}.{month}.{year}.\n\nРасчёт:\n{context}\n\nОпиши каждую энергию, её свет и тень. Дай рекомендации. Живо, как Даша, но профессионально."
            ...
            system = PERSONA_PROMPT + f"\n\nТы Даша. Настроение: {mood}. Делаешь профессиональный астрологический разбор. Женский род."
            prompt = f"Сделай астрологический разбор для {day}.{month}.{year} (знак: {zodiac}).\n\n{context}\n\nОпиши знак, планеты, дома, аспекты. Дай рекомендации. Живо, как Даша."
            ...
            system = PERSONA_PROMPT + f"\n\nТы Даша. Настроение: {mood}. Делаешь Джйотиш разбор (ведическая астрология). Женский род."
            prompt = f"Сделай Джйотиш разбор для {day}.{month}.{year}.\nЗнак (ведический): {rashi}\n\nОпиши Лагну, Грахи, Раши, Накшатры, Даши. Живо, как Даша."

        elif consult_type == "humandesign":
            zodiac = get_zodiac_sign(day, month, year)
            system = PERSONA_PROMPT + f"\n\nТы Даша. Настроение: {mood}. Делаешь разбор Дизайна Человека. Женский род."
            prompt = f"Сделай разбор Дизайна Человека для {day}.{month}.{year} (знак: {zodiac}).\n\nОпиши Тип, Стратегию, Авторитет, Профиль, Центры. Живо, как Даша."

        elif consult_type == "health":
            lp = calculate_life_path_number(day, month, year)
            system = PERSONA_PROMPT + f"\n\nТы Даша. Настроение: {mood}. Делаешь разбор здоровья (Аюрведа, психосоматика). Женский род."
            prompt = f"Сделай разбор здоровья для даты {day}.{month}.{year} (число судьбы: {lp}).\n\nОпиши Пракрити (Вата/Питта/Капха), чакры, психосоматику. Дай рекомендации. Живо, как Даша."
        else:
