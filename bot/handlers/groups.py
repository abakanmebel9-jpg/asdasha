...
logger = logging.getLogger("dasha.groups")
group_router = Router()
...
async def handle_new_members(message):
    ...
    try:
        await message.reply("Даша на связи 😊 Буду активно участвовать в беседе, ставить реакции и дополнять новости из интернета. Кидайте темы! ☕")
...
async def handle_group_photo(message):
    ...
    try:
        ...
        if data_uri:
            vision_prompt = f"Тебе прислали фото в группе. Опиши кратко что видишь (1-2 предложения), потом живо отреагируй как Даша. {'Подпись к фото: ' + caption if caption else 'Без подписи.'}"
