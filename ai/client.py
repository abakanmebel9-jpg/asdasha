"""Даша AI Client — routes all AI through OpenClaw Gateway + Pollinations direct."""
import asyncio, logging, random, time
from typing import List, Optional
import httpx
from bot.config import config

logger = logging.getLogger("dasha.ai")
...
async def _call_pollinations_direct(messages, max_tokens, timeout=30.0):
    if _client is None: await initialize()
    payload = {"model": _POLLINATIONS_MODEL, "messages": messages, "temperature": 0.9, "max_tokens": max_tokens, "stream": False, "referrer": "asdasha-bot"}
...
def _strip_name_prefix(text):
    ...
    import re
    stripped = re.sub(r'^\s*Даша\s*[:,\-—]\s*', '', text, flags=re.IGNORECASE)
